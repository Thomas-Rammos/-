...
def parse_line(line):
    ...
    pattern = re.compile(
        r'ratio=(\d+\.\d+), N=(\d+), A=(\d+)\((A\d+)\), B=(\d+)\((B\d+)\), '
        r'C=(\d+)\((C\d+)\), D=(\d+)\((D\d+)\), bestMakespan=(\d+)\((\w+)\), '
        r'OPT=(\d+\.\d+), error=(\d+\.\d+)'
    )
...
def load_data(filename):
    ...
    numeric_cols = ["ratio", "N", "A", "B", "C", "D", "bestMakespan", "OPT", "error"]
    df[numeric_cols] = df[numeric_cols].apply(pd.to_numeric, errors='coerce')

    # Group by ratio and N, keeping only numeric columns
    df = df.drop(columns=["scheduler"]).groupby(["ratio", "N"], as_index=False).mean()
